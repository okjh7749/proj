module.exports = function (app) {
    ...
    route.post('/addpost', async (req, res) => {
        ...
        route.post('/update', async (req, res) => {
            ...
            try {
                var sql0 = 'DELETE FROM Post_Tag WHERE postno = ?'
                var params0 = [postno];
                let result0 = await connection.execute(sql0, params0);
                if (result0[0].affectedRows == 0) {
                    console.log(result0[0][0]);
                }
                else {
                }
                //게시글 등록
                var sql = 'UPDATE Post SET title = ?, content = ?, writer = ? ,write_date = ? WHERE postno = ?';
                var params = [title, content, writer, write_date, postno];
                let result = await connection.execute(sql, params);
                if (result[0].affectedRows == 0) {
                    throw new Error("affectedRows is zero where post");
                    resultCode = 404;
    ...
    route.post('/delete', async (req, res) => {
        ...
        try {
            var sql0 = 'DELETE FROM Post WHERE postno = ?'
            var params0 = [postno];
            let result0 = await connection.execute(sql0, params0);
            if (result0[0].affectedRows == 0) {
                console.log(result0[0][0]);
